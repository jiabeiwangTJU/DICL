...
class Quaduplet2Lossall(nn.Module):
    ...
    def __init__(self, margin=0.3, bg_weight=0.25):
        super(Quaduplet2Lossall, self).__init__()
        self.margin = margin
    ...
    def forward(self, inputs, targets):
        ...
        for i in range(n):
            ...
            num_ap = sum(mask[i])
            num_an = sum(mask[i] == 0)
            dist_ap.append(dist[i][mask[i]].repeat(num_an))
            dist_an.append(dist[i][mask[i] == 0].repeat(num_ap))
