...
class TwoStageDetector(BaseDetector):
    ...
    def __init__(self,
        ...
        gt_roi_extractor = dict(
            ...
            roi_layer=dict(type='RoIAlign', output_size=tuple([224, 96]), sampling_ratio=0),#256  h, w[192, 64]
            out_channels=3,
            featmap_strides=[1])
    ...
    def forward_train(self,
        ...
        x = self.extract_feat(img)  ###([Batch, 1024, H/16, W/16])
        # gt_x = self.extract_feat_gt(gt_img)  ###([N_gt, 1024, 64/16, 64/16])
        gt_x = self.extract_feat(gt_img)#.detach()
        losses = dict()
        ...
        roi_losses = self.roi_head.forward_train(x, gt_x, img_metas, proposal_list,
                                                 gt_bboxes, gt_labels,
