...
def compute_new_dist(dist, all_inds, thres_dist):
    for uid in prange(11206):  # [:2]unique_inds
        for i in prange(uid + 1, 11206):
            tmp_id = (all_inds == uid).nonzero()[0] #img_ids_instance[uid]
            next_id = (all_inds == i).nonzero()[0] #img_ids_instance[i]
            temp_dist = np.empty((tmp_id.shape[0], next_id.shape[0]))
            for m in prange(tmp_id.shape[0]): #len(tmp_id)
                for n in prange(next_id.shape[0]):
                    temp_dist[m, n] = dist[tmp_id[m], next_id[n]]
            for q in prange(tmp_id.shape[0]): #len(tmp_id)
                for k in prange(next_id.shape[0]):
                    dist[tmp_id[q], next_id[k]] = dist[tmp_id[q], next_id[k]] + thres_dist * temp_dist.min()
                    dist[next_id[k], tmp_id[q]] = dist[tmp_id[q], next_id[k]]
    return dist


def clust_rank(mat, initial_rank=None, distance='cosine'):
    # print('mat', mat.shape)  ####(55272, 256)  partition0:6483
    s = mat.shape[0]
    if initial_rank is not None:
        ...
        # print('orig_dist', orig_dist[0][:10], 'initial_rank', initial_rank.max())  ###orig_dist initial_rank (55272, ) rank from min to max
    else:
    ...
    A = A.tolil()
    A.setdiag(0)
    # print('A', A.shape, 'orig_dist', orig_dist.shape)  ####(55272, 55272), (55272, 55272)
    return A, orig_dist


def clust_rank_inds(mat, inds, thres_dist, initial_rank=None, distance='cosine'):
    # print('mat', mat.shape)  ####(55272, 256)  partition0:6483
    s = mat.shape[0]
    if initial_rank is not None:
        orig_dist = []
    elif s <= ANN_THRESHOLD:
        orig_dist = metrics.pairwise.pairwise_distances(mat, mat, metric=distance) ###mat(55272, 256) orig_dist(55272, 55272)

        begin_time = time()
        orig_dist = compute_new_dist(orig_dist, inds.cpu().numpy(), thres_dist)

        np.fill_diagonal(orig_dist, 1e12)
        initial_rank = np.argmin(orig_dist, axis=1) #torch.argmin(orig_dist, dim=1).cpu().numpy()#
    ...
    # for uid in range(11206):  # [:2]unique_inds
    #     tmp_id = (inds == uid).nonzero()[0]
    #     for q in range(len(tmp_id)):
    #         if A[tmp_id[q], tmp_id] != 0:
    #             print('aa', A[tmp_id[q], tmp_id])
    #             A[tmp_id[q], tmp_id] = 0
    #             print('bb', A[tmp_id[q], tmp_id])

    # print('A', A.shape, 'orig_dist', orig_dist.shape)  ####(55272, 55272), (55272, 55272)
    return A, orig_dist

def get_clust(a, orig_dist, min_sim=None):
    # print('a', a.dtype, 'orig_dist', orig_dist.dtype)
    ...
    # print('u', u.shape, u, u.max(),  'num_clust', num_clust)  ####u[0,1,2,.....6631] num_clust 7356
    return u, num_clust
...
def req_numclust(c, data, req_clust, distance):
    ...
    for i in range(iter_):
        ...
        adj, orig_dist = clust_rank_inds(mat, inds, thres_dist, initial_rank=None, distance=distance)
...
def FINCH(data, inds, thres_dist, initial_rank=None, req_clust=None, distance='cosine', ensure_early_exit=True, verbose=True):
    ...
    adj, orig_dist = clust_rank_inds(data, inds, thres_dist, initial_rank, distance)
    initial_rank = None
    ...
    #     adj, orig_dist = clust_rank_inds(mat, inds, initial_rank, distance)
